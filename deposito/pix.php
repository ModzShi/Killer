...
if(!empty($_SESSION['email'])) {
    ...
    $checkDb=app_db();$checkStmt=app_query($checkDb,'SELECT demo FROM appconfig WHERE email=?',[(string)$_SESSION['email']]);
    $checkDemo=$checkStmt->get_result()->fetch_assoc();$checkStmt->close();$checkDb->close();
    if($checkDemo && (string)($checkDemo['demo']??'0')==='1'){header('Location: '.app_url('painel/'),true,303);exit;}
}
