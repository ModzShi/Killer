...
<head>
    ...
    <style>
        ...
        html body #game-meta-hud{min-width:min(300px,calc(100vw - 36px));padding:12px 17px;border:1px solid #ffe26d88;background:linear-gradient(135deg,#191843ed,#090e24ed);box-shadow:0 0 0 4px #7f52ff29,0 8px 32px #0009,0 0 24px #ffd43b33;backdrop-filter:blur(12px);animation:hudEnter .55s cubic-bezier(.2,1.4,.3,1) both}
        ...
        #game-tutorial button,#sair{border:1px solid #fff7!important;border-radius:15px!important;background:linear-gradient(110deg,#ffe56e,#ffb623)!important;color:#211600!important;font-weight:1000!important;box-shadow:0 6px 0 #a6670d,0 12px 28px #ffc33845!important;transition:transform .18s,filter .18s,box-shadow .18s!important;cursor:pointer}
        #game-tutorial button{width:100%;margin-top:14px;padding:16px 22px;font-size:16px}#game-tutorial button:hover,#sair:hover{transform:translateY(-3px);filter:brightness(1.08)}#game-tutorial button:active,#sair:active{transform:translateY(3px)}
        #sair{top:auto;left:50%;bottom:max(24px,env(safe-area-inset-bottom));transform:translateX(-50%);padding:12px 21px;line-height:1.2;font-size:13px;border:1px solid #fff5!important}
...
<body>
    ...
    <?php if ($gameDemo): ?><div id="game-tutorial"><div class="tutorial-card"><h2>Aprenda a jogar</h2><p>Faça uma corrida de treino em ritmo tranquilo. Aqui, todos os valores são fictícios.</p><div class="tutorial-step"><b>1</b><span>Deslize para os lados para trocar de trilho e desviar dos obstáculos.</span></div><div class="tutorial-step"><b>2</b><span>Deslize para cima para pular e para baixo para rolar. No computador, use as setas.</span></div><div class="tutorial-step"><b>3</b><span>Cada moeda vale R$ <?= number_format($gameCoinValue,2,",",".") ?>. Acompanhe a meta no topo. Esta demonstração não usa seu saldo.</span></div><button type="button" id="start-tutorial">Iniciar treino gratuito</button></div></div><?php endif; ?>
    <button id="sair" type="button">Resgatar</button>
    <script>
        window.NOSW = true;
        window.GAME_CONFIG = {
            leaderboard: 'mockup',
