    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editForm" action="/adm/usuarios/update.php" method="post">
            ...
            <label for="editSenha">Nova senha (deixe vazio para manter):</label>
            <input type="password" class="form-control" id="editSenha" name="senha" autocomplete="new-password" minlength="6" maxlength="72">
            ...
            <label for="editSaldoAfiliado">Saldo Afiliado:</label>
            <input type="text" class="form-control" id="editSaldoAfiliado" name="comissaofake" >
            ...
            <br/>
            <label for="editAfiliadoAtivo">Afiliado Ativo (Revenue Share):</label>
            <input type="checkbox" id="editAfiliadoAtivo" name="afiliado_ativo" >


            <br style='margin-bottom: 15px'>
            <input type="hidden" id="editDepositou" name="depositou">
            <input type="hidden" id="editSaldoComissao" name="saldo_comissao">
            <input type="hidden" id="editPercas" name="percas">
            <input type="hidden" id="editGanhos" name="ganhos">
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        data.forEach(function(row) {
            var newRow = "<tr>" +
            "<td>" + escapeHtml(row.data_cadastro) + "</td>" +
            "<td>" + escapeHtml(row.email) + "</td>" +
            "<td>" + escapeHtml(row.telefone) + "</td>" +
            "<td>" + escapeHtml(row.saldo) + "</td>" +
         
            "<td>" + escapeHtml(row.depositou) + "</td>" +
            "<td>" + escapeHtml(row.indicados) + "</td>" +
            "<td>" + escapeHtml(row.cpa) + "</td>" +
            "<td>" + escapeHtml(row.plano) + "</td>" +
           
            "<td><button class='btn-edit' data-id='" + escapeHtml(row.id) + "'>Editar</button></td>" +
            "</tr>";
        ...
        function fillEditModal(userId) {
            ...
            $('#editSaldoAfiliado').val(user.comissaofake);
            ...
            $('#editAfiliadoAtivo').prop('checked', user.afiliado_ativo === '1');
            $('#editDepositou').val(user.depositou);
            $('#editSaldoComissao').val(user.saldo_comissao);
            $('#editPercas').val(user.percas);
