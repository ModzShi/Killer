...
try{
    ...
    check_manager(str_contains($html,'QA Gerente')&&str_contains($html,'Influenciador QA')&&str_contains($html,'Contas demo'),'Manager dashboard renders its own data');
}finally{
    ...
    app_query($db,'DELETE FROM manager_demos WHERE email=?',[$demoEmail]);
    app_query($db,'DELETE FROM appconfig WHERE email IN (?,?)',[$leadEmail,$demoEmail]);
